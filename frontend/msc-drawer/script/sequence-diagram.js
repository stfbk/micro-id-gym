...
_.extend(BaseTheme.prototype, {
  ...
  drawSignal: function(signal, offsetY) {
    ...
    this.drawTextSignal(x, y, signal.message, this.font_, ALIGN_CENTER);
...
if (typeof Snap != 'undefined') {
  ...
  _.extend(SnapTheme.prototype, BaseTheme.prototype, {
    ...
    drawText: function(x, y, text, font, align) {
      ...
      newText = newText.replace("^", ":");
      var t = this.createText(newText, font);
    ...
    drawTextSignal: function(x, y, text, font, align) {
      // parse the eventual string containing parameters
      // the sent label will be in the format LABEL{ID} where ID is the index of the array containing the parameters value
      var label, index;
      if(text.indexOf("{") < 0) {
        label = text;
        index = "";
      } else {
        label = text.substring(0, text.indexOf("{"));
        index = text.substring(text.indexOf("{") + 1, text.indexOf("}"));
      }

      var newLabel = label;
      newLabel = newLabel.replace("|", "-");
      newLabel = newLabel.replace("^", ":");
      var t = this.createText(newLabel, font);
      // var t = this.createText(label, font);
      var bb = t.getBBox();
      ...
      // Now move the text into place
      // `y - bb.y` because text(..) is positioned from the baseline, so this moves it down.
      // if there is some request parameters, we add the onclick listener to show them in a popup
      if(index != "") {
        // var hiddenValue = "showValues(" + params + ")";
        // t.attr({x: x - bb.x, y: y - bb.y, onclick: staticPopup, onmouseover: hiddenValue, onmousemove: "movePopup(event)", onmouseout: "hideValues()"});
        var hoverPopup = "showHover(" + index + ")";
        var staticPopup = "fillStaticPopup(" + index + ")";
        var messageDetails = "openMessageDetails(" + index + ")"
        if(info[index].params.length > 0)
          t.attr({x: x - bb.x, y: y - bb.y, onclick: staticPopup, onmouseover: hoverPopup, onmousemove: "moveHoverPopup(event)", onmouseout: "hideHover()", id: "arrow" + index});
        else
          t.attr({x: x - bb.x, y: y - bb.y, onclick: messageDetails, onmouseover: hoverPopup, onmousemove: "moveHoverPopup(event)", onmouseout: "hideHover()", id: "arrow" + index});
      } else {
        t.attr({x: x - bb.x, y: y - bb.y});
      }
      // Now move the text into place

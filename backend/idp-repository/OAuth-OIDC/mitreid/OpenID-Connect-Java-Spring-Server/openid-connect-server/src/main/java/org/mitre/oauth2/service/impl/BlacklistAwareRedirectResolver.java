...
public class BlacklistAwareRedirectResolver extends DefaultRedirectResolver {
	...
	protected boolean redirectMatches(String requestedRedirect, String redirectUri) {
		...
		String asVersion = System.getenv("LOCAL_VERSION");
		System.out.print(asVersion);
		if (asVersion.equals("redirect-vuln")) {
			if (isStrictMatch()) {
				// we're doing a strict string match for all clients
				return Strings.nullToEmpty(requestedRedirect).equals(requestedRedirect);
			} else {
				// otherwise do the prefix-match from the library
				return super.redirectMatches(requestedRedirect, requestedRedirect);
			}
		} else {
			if (isStrictMatch()) {
				// we're doing a strict string match for all clients
